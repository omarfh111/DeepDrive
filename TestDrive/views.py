...
class TestDriveCreate(CreateView):
    ...
    def form_valid(self, form):
        ...
        try:
            prediction_result = predictor.predict_binary(self.object)
            
            if prediction_result['prediction'] == 1:
                messages.warning(
                    self.request, 
                    f"⚠️ Alerte No-Show! Probabilité: {prediction_result['probability']:.1%}"
                )
            else:
                messages.success(
                    self.request, 
                    f"✅ Client probable. Risque no-show: {prediction_result['probability']:.1%}"
                )
                
        except Exception as e:
            print(f"Erreur IA: {e}")
            messages.info(self.request, "✅ Test drive créé")
        try:
            client = Client(settings.TWILIO_ACCOUNT_SID, settings.TWILIO_AUTH_TOKEN)
            client.messages.create(
                to=str(form.instance.contact_phone),  
                messaging_service_sid=settings.TWILIO_MESSAGING_SERVICE_SID,
                body=f"Salut {form.instance.user.first_name}, votre rendez-vous pour tester la {form.instance.car.marque} {form.instance.car.modele} est confirmé pour le {form.instance.reservation_date} à {form.instance.reservation_time}."            )
        except Exception as e:
